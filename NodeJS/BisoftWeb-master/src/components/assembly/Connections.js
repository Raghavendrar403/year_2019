...
var componentType = localStorage.getItem('biboxTypes');
 ...
 if(componentType=="*STA#"){
  ...
  render: function () {
    ...
    cx2 = LeftS + Ports[connectedTo][0];
    cy2 = TopS + Ports[connectedTo][1];
    cx2 += Ports[connectedTo][0] - ImageSizes[ItemTypes.COMPONENT][0]/2;
    cy2 += Ports[connectedTo][1] - ImageSizes[ItemTypes.COMPONENT][1]/2;
       return (
      <path d={'M ' + (left + ImageSizes[ItemTypes.COMPONENT][0]/2) +
                ...
                ' ' + cx2 +
                ',' + cy2 +
                ' ' + (LeftS + Ports[connectedTo][0]) +
                ',' + (TopS + Ports[connectedTo][1])}/>
    
    );
    }
    else if(connectedTo=="A33" ){

       Object.keys(components).map(servo_extender => {
                    if(servo_extender=="servo_extender"){
                       components[servo_extender].map((component, index) => {
                          if(component.connectedTo=="A3"){
                          LeftSS = component.left;
                          TopSS  = component.top;
    ...
    cx2 = LeftSS + Ports[connectedTo][0];
    cy2 = TopSS + Ports[connectedTo][1];
    cx2 += Ports[connectedTo][0] - ImageSizes[ItemTypes.COMPONENT][0]/2;
    cy2 += Ports[connectedTo][1] - ImageSizes[ItemTypes.COMPONENT][1]/2;
       return (
      <path d={'M ' + (left + ImageSizes[ItemTypes.COMPONENT][0]/2) +
                ...
                ' ' + cx2 +
                ',' + cy2 +
                ' ' + (LeftSS + Ports[connectedTo][0]) +
                ',' + (TopSS + Ports[connectedTo][1])}/>
