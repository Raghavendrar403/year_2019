  ...
  AllowedPortTypes[component.type].some(portType => {
    return PortTypes[portType].ports.some(port => {
      ...
      if(port=="F1" || port=="F2"){
          ...
          Object.keys(componentConnected).map(servo_extender => {
               if(servo_extender=="servo_extender"){
                       componentConnected[servo_extender].map((componentConnected, index) => {
                          if(componentConnected.connectedTo=="A2"){
                          LeftSM = componentConnected.left;
                          TopSM = componentConnected.top;
                          

                       l = Ports[port][0] +LeftSM,
         
                        t = Ports[port][1] + TopSM;
          ...
          Object.keys(componentConnected).map(servo_extender => {
               if(servo_extender=="servo_extender"){
                       componentConnected[servo_extender].map((componentConnected, index) => {
                          if(componentConnected.connectedTo=="A3"){
                          LeftSSM = componentConnected.left;
                          TopSSM = componentConnected.top;
                          

                       l = Ports[port][0] +LeftSSM,
         
                        t = Ports[port][1] + TopSSM;
